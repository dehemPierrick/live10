...
// on défini arbitrairement une phrase à comparer en premier
indexLongest = 0;
...
for (var index = 0; index < phrases.length; index++) {
    html +=
        ...
        index + " : " +
        phrases[index] + " (" +
        phrases[index].length + ")" +
        '</li>';

    // a chaque itération de la boucle on compare la longueur actuelle avec
    // la longueur de la phrase considéré comme la plus longue
    if (phrases[index].length > phrases[indexLongest].length) {
        indexLongest = index;
    }
}
...
/*******************************************
 Display results
 *******************************************/
html += '<p>La phrase la plus longue fait ' + phrases[indexLongest].length + ' caractères</p>';
html += "<p>c'est : <em>" + phrases[indexLongest] + '</em></p>';

console.log(phrases[indexLongest].length);
